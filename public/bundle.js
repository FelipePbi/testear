    ...
    3: [
      function (require, module, exports) {
        ...
        AFRAME.registerComponent("osm", {
          ...
          update: function () {
            if (
              this.data.latitude >= -90 &&
              this.data.latitude <= 90 &&
              this.data.longitude >= -180 &&
              this.data.longitude <= 180
            ) {
          ...
          _readOsm: function (lat, lon) {
            fetch(
              `https://hikar.org/fm/ws/bsvr.php?bbox=${lon - 0.01},${lat - 0.01},${lon + 0.01},${
                lat + 0.01
              }&way=highway&format=json&outProj=3785`
            )
                ...
                json.features.forEach((f, i) => {
                  ...
                  if (f.geometry.type == "LineString" && f.geometry.coordinates.length >= 2) {
                    ...
                    if (line.length >= 2) {
                      const g = new OsmWay(
                        line,
                        drawProps[f.properties.highway] ? drawProps[f.properties.highway].width || 5 : 5
                      ).geometry;

                      const color = drawProps[f.properties.highway]
                        ? drawProps[f.properties.highway].color || "#ffffff"
                        : "#ffffff";
